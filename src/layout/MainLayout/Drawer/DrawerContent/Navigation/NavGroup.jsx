...
const NavGroup = React.forwardRef(
  ({ item, lastItem, remItems, lastItemId, setSelectedItems, selectedItems, setSelectedLevel, selectedLevel }, ref) => {
    ...
    const navCollapse = item.children?.map((menuItem, i) => {
      switch (menuItem.type) {
        ...
        case 'item':
          return <NavItem key={i} item={menuItem} level={1} />;
        default:
      }
    });
    ...
    const items = currentItem.children?.map((menu, i) => {
      switch (menu.type) {
        case 'collapse':
          return (
            <NavCollapse
    ...
    const popperId = openMini ? `group-pop-${1}` : undefined;
