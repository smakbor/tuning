...
const NavCollapse = React.forwardRef(({ menu, level, parentId, setSelectedItems, selectedItems, setSelectedLevel, selectedLevel }, ref) => {
  ...
  const navCollapse = menu.children?.map((item, i) => {
    switch (item.type) {
      case 'collapse':
        return (
          <NavCollapse
      ...
      case 'item':
        return <NavItem key={i} item={item} level={level + 1} />;
      default:
        return (
