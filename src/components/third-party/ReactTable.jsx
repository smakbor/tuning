...
import { useSetTableHiddenRowMutation } from 'api/service/tableRowHidden.service';
...
export const TablePagination = ({ gotoPage, isServerPagination, setPageSize, pageSize, pageIndex, setPageIndex, rows, totalPage }) => {
  // ====================== PACKAGE HOOKS ======================
  ...
  return (
    <Grid container alignItems="center" justifyContent="space-between" sx={{ width: 'auto' }}>
      <Grid item>
        <Stack direction="row" spacing={1} alignItems="center">
          ...
          <TextField
            ...
            inputProps={{ min: 1, max: isServerPagination ? totalPage : Math.ceil(rows.length / pageSize) }}
            value={isServerPagination ? Number(page) : pageIndex}
            onChange={(e) => {
              handleGotoPage(Number(e.target.value) === 0 ? 1 : Number(e.target.value));
            }}
      ...
      <Grid item sx={{ mt: { xs: 2, sm: 0 } }}>
        <Pagination
          // eslint-disable-next-line react/prop-types
          count={isServerPagination ? totalPage : Math.ceil(rows.length / pageSize)}
          page={isServerPagination ? Number(page) || 1 : pageIndex}
...
export const HidingSelect = ({ hiddenColumns, setHiddenColumns, allColumns, rowHiddenName }) => {
  const { user } = useAuth();
  // >==========|| RTK QUERY AND MUTATION HOOK ||===========
  const [setHiddenDataInBD, { isLoading }] = useSetTableHiddenRowMutation();
  ...
  // =====< || HANDLER SET HIDDEN COLUMN || >=====
  const setHiddenDataInBDHandler = () => {
    setHiddenDataInBD({ data: { [rowHiddenName]: hiddenColumns, merchant: user.merchant }, merchant: user.merchant, rowHiddenName });
  };
  ...
  return (
    <FormControl sx={{ width: 110 }}>
      <Select
        ...
        renderValue={(selected) => {
          if (selected.length === 0) {
            return <Typography variant="subtitle1">Visibility</Typography>;
          }
      ...
      >
        ...
        <Stack sx={{ position: 'sticky', bottom: 0 }} direction="row" justifyContent="center" alignItems="center">
          <LoadingButton
            size="small"
            sx={{ width: '100%' }}
            onClick={setHiddenDataInBDHandler}
            loading={isLoading}
            loadingPosition="end"
            variant="contained"
          >
